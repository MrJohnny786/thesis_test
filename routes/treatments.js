...
router.get("/new", middleware.isLoggedIn, function (req, res) {
  ...
  Diagnose.findById(req.params.diagnose_1, function (err, diagnose) {
    if (diagnose.treatments.length > 0) {
      findTreat = diagnose.treatments[diagnose.treatments.length - 1];
      Treatment.findById(findTreat, function (err, lastTreatment) {
        lastTreatmentDate = lastTreatment.date.toISOString().split("T")[0];
      });
    } else {
      lastTreatmentDate = diagnose.date.toISOString().split("T")[0];
    }
    Staff.find({ role: "Νοσηλευτης" }, function (err, findStaff) {
      if (err) {
        ...
        res.render("treatments/new", {
          ...
          diagnose: diagnose,
          hosp_staff: findStaff,
          latestTreatmentDate: lastTreatmentDate,
        });
      }
    });
  });
        ...
        if (err) {
          ...
          Treatment.create(req.body.treatment, function (err, treatment) {
            if (err) {
              ...
              diagnose.save();
              res.redirect(
                "/patients/" + patient._id + "/diagnoses/" + diagnose.id
              );
            }
          });
        }
...
router.get("/:treatment_id", function (req, res) {
  Treatment.findById(req.params.treatment_id, function (err, foundTreatment) {
    if (err) {
      ...
      res.render("treatments/show", {
        patient_id: req.params.id,
        diagnose_id: req.params.diagnose_1,
...
router.get(
  "/:treatment_id/edit",
  middleware.checkTreatmentOwnership,
  function (req, res) {
    Treatment.findById(req.params.treatment_id, function (err, foundTreatment) {
      Staff.find({ role: "Νοσηλευτης" }, function (err, findStaff) {
        if (err) {
...
router.delete(
  ...
  function (req, res) {
    Diagnose.updateOne(
      { _id: req.params.diagnose_1 },
      { $pull: { treatments: new ObjectId(req.params.treatment_id) } },
      function (err) {
        if (err) {
          console.log(err);
        } else {
