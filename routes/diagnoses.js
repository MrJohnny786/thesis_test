...
router.get("/:diagnose_id", function (req, res) {
  Diagnose.findById(req.params.diagnose_id)
    .populate("treatments")
    .exec(function (err, foundDiagnose) {
      if (err) {
        res.redirect("/patients");
      } else {
        res.render("diagnoses/show", {
          patient_id: req.params.id,
          diagnose: foundDiagnose,
        });
      }
    });
});
