$(document).ready(function () {
  ...
  $(".showForm").on("click", function () {
    let text = $(".showForm").text().trim();
    if (text === "Προσθηκη φορμας παρενεργειων") {
      $(".showForm").text("Αφαίρεση φορμας παρενεργειών");
      $(".effectsForm").toggle();
    } else {
      $(".showForm").text("Προσθηκη φορμας παρενεργειων");
      $(".effectsForm").toggle();
    }
  });
      ...
      success: function (response) {
        ...
        if (response.msg == "success") {
          $.each(response.data, function (index, data) {
            ...
            $(".mygrid").append(
              '<div class="col-sm-2 mt-2"> <table class="table-responsive-sm "> <thead class="col-sm"> <tr class="col-sm text-nowrap"><th class="col-sm" scope="col">' +
                daty +
                ' <a class="btn btn-sm btn-outline-success " href="/effects/' +
                data._id +
                '/edit">Ε</a><a class="btn btn-sm btn-outline-danger deletus" data-id=' +
                data._id +
                ' href="/effects/removeEffect">Δ</a></th> </tr> </thead><tbody class="col-sm addTr' +
                index +
                '"></tbody> </div>'
            );
            ...
            for (const [key, value] of Object.entries(data)) {
              if (
                typeof value === "object" &&
                value != null &&
                value.hasOwnProperty("effects")
              ) {
                for (const [key1, value1] of Object.entries(value.effects)) {
                  if (value1) {
                    const cc = ".addTr".concat(index);
                    $(cc).append(
                      '<tr class="col-sm"><td class="col-sm">' +
                        names[key].effects[key1].name +
                        "</td></tr>"
                    );
                  } else {
                    continue;
  ...
  $(".addbtn1").click(function (e) {
    ...
    const datepick2 = $(".daty").val();
    const datepick = $(".effectdate").val();
    idata.date = datepick !== undefined ? datepick : datepick2;
    $.ajax({
      url: "/effects/add",
      method: "post",
      dataType: "json",
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.msg === "success") {
          ...
          window.location =
            "/patients/" + p_id + "/diagnoses/" + t_id + "/treatments/" + t_id;
        } else {
